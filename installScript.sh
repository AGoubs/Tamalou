...
cd /opt 
wget https://www.python.org/ftp/python/3.9.6/Python-3.9.6.tgz -y
tar xzf Python-3.9.6.tgz 
cd Python-3.9.6 
./configure --enable-optimizations -y
make altinstall -y
rm -f /opt/Python-3.9.6.tgz
pip3.9 install -U spacy -y
pip3.9 install -U pandas -y
python3.9 -m spacy download fr_core_news_lg -y